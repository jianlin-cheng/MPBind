...
def get_pdb_xyz(pdb_file):
    ...
    for line in pdb_file:
        # print(f"line:0-4: {line[0:4].strip()} and pos: {line[22:26].strip()} and Atom: {line[12:16].strip()}")
        if (line[0:4].strip() == "ATOM" and int(line[22:26].strip()) != current_pos) or line[0:4].strip() == "TER":   # TER indicates the end of a chain
            if current_aa != {}:
                atom_mm =  ["N", "CA", "C", "O"]
                cur_mean = None
                for aa in atom_mm:
                    if aa in current_aa:
                        cur_mean = current_aa[aa]
                        break
                # print(f"the current mean: {cur_mean}\n")
                if not isinstance(cur_mean, np.ndarray):
                    for aa in current_aa:
                        cur_mean = current_aa[aa]
                        break
        
                for aa in atom_mm:
                    if aa not in current_aa:
                        current_aa[aa] = cur_mean
        ...
        if line[0:4].strip() == "ATOM":
            atom = line[12:16].strip()  # here should be [12:16] rather than [13:16]
            if atom != "H":
